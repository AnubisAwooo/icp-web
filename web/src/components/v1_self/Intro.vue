<template>
  <header class="App-header">
    ...
    <button class="demo-button" @click="increment1()">
      Count is: {{ count1 }}
    </button>
...
import { icp_web } from "canisters/icp_web"
...
export default {
  ...
  setup: () => {
    ...
    const refreshCounter1 = async () => {
      const res: any = await icp_web.getValue()
      count1.value = res.toString()
    }

    const increment1 = async () => {
      await icp_web.increment()
      refreshCounter1()
    }

    onMounted(refreshCounter1)

    return { increment1, count1 }
  },
}
</script>
